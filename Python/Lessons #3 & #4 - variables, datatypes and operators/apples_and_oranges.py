...
def add_oranges(amount):
    ...
    try:
        while (amount > 0):
            if amount == 1:
                canvas.create_image(user_screen_width / 2 + 20, user_screen_height / 2 + 20, image=orange_img, anchor=tk.CENTER)
            else:
...
def add_apples(amount):
    ...
    try:
        while (amount > 0):
            if amount == 1:
                canvas.create_image(user_screen_width / 2, user_screen_height / 2, image=apple_img, anchor=tk.CENTER)
            else:
