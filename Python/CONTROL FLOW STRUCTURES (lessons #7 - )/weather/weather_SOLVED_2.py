...
images: dict = pygame_utils.creates_images({"sunny_scaled_surface": ["sunny.png", "center", screen_center, x_and_y],
...
while(running == True):
    ...
    if weather == "sunny":
        screen.blit(images["sunny_scaled_surface"], sunny_centered_rect)

    else:       # weather == "rainy"
        screen.blit(images["rainy_scaled_surface"], rainy_centered_rect)
        screen.blit(images["base_scaled_surface"], base_centered_rect)     # Draw_base()

        if draw_umbrella == True:
            screen.blit(umbrella_scaled_surface, images["umbrella_scaled_surface"])

        else:
            screen.blit(images["sad_face_scaled_surface"], sad_face_centered_rect)
